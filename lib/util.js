        ...
        return request.execute('sp_tables').
            then(function (res) {
                ...
                for (var i = 0; i < tableList.length; i++) {
                    ...
                    tables.push({ name: tableList[i].TABLE_NAME, owner: tableList[i].TABLE_OWNER });
                }
            })
    },
    exportTable: function (tableOwner, tableName, outputDirectory) {
        return new Promise(function (resolve, reject) {
            var csvStream = csv.format({ headers: false, quoteColumns: true }),
                writableStream = fs.createWriteStream(outputDirectory + "/" + tableOwner + '.' + tableName + ".csv", { encoding: "utf8" });
            ...
            var request = new sql.Request();
            request.stream = true;
            request.query('select * from ' + tableOwner + '.' + tableName);
